...
// Match any inequality operator
const INEQ = String.raw`>=|<=|>|<`;
...
export function parseInequality(input) {
  ...
  const ineqMatch = s.match(new RegExp(`(${INEQ})`));
  if (!ineqMatch) return null;
  const ineq = ineqMatch[1];

  // Split on the operator
  const parts = s.split(new RegExp(`(${INEQ})`));
  if (parts.length < 3) return null;
  let left = parts[0];
  let right = parts.slice(2).join("");

  // Check for flipped format: c > expr
  let flipped = false;
  let flipIneq = ineq;
  if (!(/[a-zA-Z]/.test(left)) && /[a-zA-Z]/.test(right)) {
    flipped = true;
    // Flip the inequality: 8 > x+3  →  x+3 < 8
    const flipMap = { ">": "<", "<": ">", ">=": "<=", "<=": ">=" };
    flipIneq = flipMap[ineq];
  ...
  if (divMatch) {
    ...
    return {
      type: "multiply", inequality: flipIneq,
      boxValue: xVal, initBoxes: 1, initPills: 0, initSlots: 0,
      rightPills: c, divisor: d, variable: v, flipped,
    };
  }

  // ax ± b = c  (a may be negative, e.g. -5x < 10)
  ...
  const base = { inequality: flipIneq, boxValue: xVal, variable: v, flipped, ...(negCoeff && { negCoeff: true }) };
...
export function levelIneq(l) {
  ...
  return l.flipped ? `${numSide} ${sym} ${varSide}` : `${varSide} ${sym} ${numSide}`;
}

export { gcd, INEQ_DISPLAY };
