...
// Convert distribution level to a standard equation level (post-distribution form)
...
export function useDistributionExplore(level) {
  ...
  // Phase: "distribute" | "left" | "right" | "unbalanced" | "solved"
  ...
  function applyToLeft(op, n) {
    ...
    if (result.error) { setLeftError(result.error); return; }
  ...
  function applyToRight(op, n) {
    ...
    if (op === leftOp && n === leftNum) {
      ...
      if (!newLeft.error && isSolved(newLeft)) {
        setPhase("solved");
        const solvedText = `${varName} = ${fmt(newR)}`;
        setWorkLines((w) => [
          ...w.slice(0, -2),
          { type: "op", op, n },
  ...
  return {
    ...eq,
    phase, distributed,
    leftOp, leftNum, leftError, rightError,
    snap, opKey, workLines,
    initialEq,
    distribute,
